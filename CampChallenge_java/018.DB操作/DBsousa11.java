 ...
 * @author rui
 */
public class DBsousa11 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            db_ins = db_con.prepareStatement("update profiles set name = ?,tell = ?,age=? where profilesID = ?");
...
//                String birth = request.getParameter("birthdayT");                
            db_ins.setString(1,name);//１個めの？に入れる値の指定
            db_ins.setString(2,tell);//２個めの？に入れる値の指定
            db_ins.setInt(3,age);//３個めの？に入れる値の指定
            db_ins.setInt(4,ID);//４個めの？に入れる値の指定
